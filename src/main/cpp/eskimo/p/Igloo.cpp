...
Igloo::Igloo(GVars *ss) {
  ...
  auto onGravityChanged = [=](EventCustom*) {
    _block->setSpriteFrame(
        cx::getSpriteFrame("block_large_" +
          s::to_string( (int) ss->gravity) + ".png"));
  };

  auto onLevelCompleted = [=](EventCustom*) {
    this->setDisplayFrame(cx::getSpriteFrame("igloo_on.png"));
  };
...
void Igloo::initIgloo(int gravity, const c::Vec2 &pos) {
  ...
  _block->setDisplayFrame(
      cx::getSpriteFrame(
        "block_large_" + s::to_string(ss->gravity) + ".png"));
